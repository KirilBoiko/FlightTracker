...
Система опирается на три различных скрипта для сбора данных и визуализатор аналитики, каждый из которых служит своей конкретной цели:
...
### 3. Скрейпер аэропортов Flightradar24 (`fr24_airport_scraper.py`)
**Цель:** Извлечение точных данных с табло вылетов и прилетов для конкретного аэропорта (например, TBS) и фильтрация определенных маршрутов рейсов.

**Как это работает:**
Flightradar24 надежно защищен с помощью Cloudflare.
1. Этот скрипт обходит Cloudflare, направляя запросы через премиальные резидентные прокси ScrapingBee.
2. Он обращается к скрытому внутреннему JSON API Flightradar24 вместо рендеринга HTML, извлекая высокоструктурированные данные (номера рейсов, авиакомпании, время по расписанию, типы самолетов).
3. Он перебирает несколько дат (например, Вчера, Сегодня, Завтра) и обрабатывает как *вылеты* (departures), так и *прилеты* (arrivals).
4. Он анализирует массивы JSON и фильтрует данные так, чтобы в итоговом CSV-файле содержались только рейсы, прибывающие из нашего целевого аэропорта или вылетающие в него (например, Тель-Авив / TLV).

### 4. Визуализатор аналитики цен (`plot_price_history_v2.py`)
...
### Запуск Скрейпера Flightradar24
Чтобы собрать отфильтрованные данные табло вылетов и прилетов для TBS, запустите:
```bash
python3 fr24_airport_scraper.py
```
*   **Чего ожидать:** Скрипт запросит API за Вчера, Сегодня и Завтра. Он выведет информацию о том, сколько рейсов он нашел по целевому фильтру (TLV) для каждой даты, и создаст чистый CSV-файл (`fr24_tbs_tlv_filtered_flights.csv`).
...
**Чтобы изменить это в `fr24_airport_scraper.py`:**
Загляните внутрь функции `main()` примерно на 117 строке, чтобы изменить целевые даты, или отредактируйте константы в начале файла для выбора целевого аэропорта:
```python
TARGET_AIRPORT = "tbs"
FILTER_IATA = "TLV"
```
